  ...
  create or replace view `chessthesis`.`chess_thesis`.`mrt_player_stat_with_historical_for_game`
  OPTIONS()
  as with player_stat_for_game as (
    select 
        game_id,
        player_id,
        player_side,
    ...
    from (
        ...
        inner join `chessthesis`.`chess_thesis`.`mrt_player_stats` ps on ps.game_id = tg.id
        inner join `chessthesis`.`chess_thesis`.`mrt_game_stats` gs on gs.game_id = tg.id
