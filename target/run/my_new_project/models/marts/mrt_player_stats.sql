  ...
  create or replace view `chessthesis`.`chess_thesis`.`mrt_player_stats`
  OPTIONS()
  as with player_stats_by_game as (
    select 
        game_id,
    ...
    from `chessthesis`.`chess_thesis`.`mrt_gamepositions_with_evaluations`
...
players_stats_extended as (
   select 
    ...
    left join `chessthesis`.`chess_thesis`.`mrt_filtered_twicgames` tg on tg.id = psbg.game_id
)
